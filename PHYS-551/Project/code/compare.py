...
from importance_sampling import SAMPLES_PER_THETA, THETA_LADDER

IS_N = len(THETA_LADDER) * SAMPLES_PER_THETA   # pooled MIS sample count


def _naive_n(naive):
    """Infer the naive sample count from the CSV (hits / probability)."""
    m = naive["Hits"] > 0
    return int(round((naive["Hits"][m] / naive["Exceedance_Probability"][m]).median()))
...
def ep_curve(naive, is_ep):
    ...
    ax.errorbar(naive["Threshold_People"][seen] / 1e6,
                ...
                label=f"Naive MC (N={n_naive:,})", zorder=4)
...
def convergence():
    ...
    ax.set_title(f"Convergence on the rarest bucket (≥ {CONVERGENCE_THRESHOLD/1e6:.0f}M people)\n"
                 "Naive jumps on its few chance hits; IS converges smoothly", fontsize=13, fontweight="bold")
    ax.grid(True, which="both", ls=":", alpha=0.5)
...
def variance_table(naive, is_ep):
    ...
    for _, nr in naive.iterrows():
        ...
        var_is_ps = (ir["SE"] ** 2) * IS_N                   # SE^2 * pooled N
