...
Model definition: districts, loads, safety margins (alpha), redistribution
factors (gamma), adjacency, and the demand-volatility constants.
"""
import os
import json
import numpy as np
...
#
# mu_i is each district's BASE load (a fixed scale derived from its share of
# regional consumption). On a given simulated day the demand is
#       L_i = mu_i * (S + eps_i)
# where
#   S      ~ Normal(1, VOL_SYS)   -- a CITY-WIDE demand multiplier shared by every
#                                    district (a heatwave turns on every AC at
#                                    once); a large S is the rare event that drives
#                                    a system-wide cascade.
#   eps_i  ~ Normal(0, VOL_IDIO)  -- small district-specific noise, independent.
#
# A district trips when L_i > capacity_i = mu_i*(1+alpha_i), i.e. when
# S + eps_i > 1 + alpha_i. Because capacity and demand both scale with mu_i, the
# model is scale-free: only the dimensionless ratios matter and the absolute load
# unit drops out of every result. Almost all tail risk comes from S, which is why
# Importance Sampling biases S alone (a clean 1-D tilt).
# ----------------------------------------------------------------------------
VOL_SYS  = 0.045  # std of the city-wide demand multiplier S
VOL_IDIO = 0.02   # std of the district idiosyncratic demand noise eps_i
...
if os.path.exists(stats_file):
    ...
    print(f"⚠️  WARNING: {stats_file} not found — every district falls back to the "
          f"maximum safety margin (ALPHA_MAX), so the risk results will be wrong. "
          f"Run fetch_outage_data.py to use the real overload data.")
...
# Calibrated so even the weakest district sits safely below capacity on an average
# day and only fails under a rare city-wide surge (alpha=0.08 means it trips once
# S + eps_i exceeds 1.08).
...
for name, data in ISTANBUL_ALL_LAND_NODES.items():
    ...
    if side_max > 0:
        # alpha_i = ALPHA_MAX - (count_i / max_count_on_side) * (ALPHA_MAX - ALPHA_MIN)
