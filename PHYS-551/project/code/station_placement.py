...
from importance_sampling import THETA_LADDER
...
def eval_is_ep(grid, thetas=THETA_LADDER, n=IS_SAMPLES):
    """Importance-sampling exceedance curve for the grid. Returns {threshold: (P, SE)}."""
    names = grid["names"]
    pop = grid["pop"]
    K = len(names)
    per_theta = []
    for theta in thetas:
        ...
        for _ in range(n):
            ...
            S = 1.0 + VOL_SYS * z
            W = np.exp(-theta * z + half_t2)
            eps = np.random.normal(0.0, VOL_IDIO, size=K)
            status = simulate_cascade(_sample_loads(grid, S, eps),
                                      grid["nodes"], grid["adjacency"], grid["gamma"])
            affected = sum(pop[j] for j, nm in enumerate(names) if status[nm] == 0)
        ...
        for t in EXCEEDANCE_THRESHOLDS:
            m1 = exc_w[t] / n
            m2 = exc_w2[t] / n
            ep[t] = (m1, np.sqrt(max(0.0, (m2 - m1 * m1) / n)))
        per_theta.append(ep)
    ...
    for t in EXCEEDANCE_THRESHOLDS:
        ...
        for ep in per_theta:
            P, SE = ep[t]
            if P > 0 and (best is None or SE < best[1]):
                best = (P, SE)
        stitched[t] = best if best else (0.0, 0.0)
    return stitched
