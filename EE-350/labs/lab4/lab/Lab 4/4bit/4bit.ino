/*
 * REAL HARDWARE SAR ADC (4-BIT VERSION)
 * -------------------------------------
 * HARDWARE:
 ...
 * * USAGE:
 * - Send 'b': Begin conversion (Starts at Mid-Scale)
 * - Send 'n': Next step (Bit by bit decision)
 */
...
// --- Global Variables ---
int digitalValue = 0;
int currentBit = 3; // MSB for 4-bit is Bit 3
bool conversionInProgress = false;

// PWM Timing
const int pwm_step_us = 640; 
const int MAX_VAL = 16; // 2^4

// Helper: Print 4-Bit Binary
...
void setup() {
  ...
  Serial.println("System Ready (4-Bit). Send 'b' to start.");
}
...
void loop() {
  ...
  // Safety checks to avoid 0 delay
  if (digitalValue > 0) {
  ...
  if (Serial.available()) {
    ...
    if (cmd == 'b') {
      ...
      currentBit = 3;
      digitalValue = (1 << currentBit); // Start with MSB (1000) = 8

      Serial.println("\n--- STARTING CONVERSION ---");
      
      // İlk adımın voltajını yazdır
      float currentPwmVoltage = digitalValue * (5.0 / (float)MAX_VAL);
      Serial.print("Bit 3 | Reg: ");
      printBinary(digitalValue);
      Serial.print(" | PWM Voltaj: ");
      Serial.print(currentPwmVoltage, 3);
      Serial.println(" V");
